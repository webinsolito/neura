...
func TestReasoningLearningV1Benchmark(t *testing.T) {
    ...
    for i, plan := range cases {
        ...
        for _, r := range res.Results { if r.Verified { verified++ } }
    }
    ...
    candidate, err = mgr.RecordEvaluation(candidate.ID, ImprovementEvaluation{
        TestsPassed: true, BenchmarkMeasured: true, BenchmarkNoWorse: true,
        Evidence: []string{"deterministic reasoning-learning benchmark"},
    })
    if err != nil { t.Fatal(err) }
    if !candidate.PromotionReady { t.Fatal("verified candidate should be promotion-ready") }
    ok, _, err := mgr.PromotionDecision(candidate.ID)
    if err != nil { t.Fatal(err) }
    if !ok { t.Fatal("governed learning gate rejected verified candidate") }
}
