...
func TestExecutePlanPrevalidatesBeforeAnySideEffect(t *testing.T) {
	...
	p := Plan{Summary:"must reject whole plan", Steps:[]PlanStep{
		{Tool:"fs.write.workspace", Input:map[string]string{"path":"should-not-exist.txt","content":"x"}},
		{Tool:"shell.exec", Input:map[string]string{"command":"bad"}},
	}}
	r := c.ExecutePlan(context.Background(), "test prevalidation", p)
	if r.Status != "blocked" { t.Fatalf("%+v", r) }
	if _, err := os.Stat(filepath.Join(tools.workspace, "should-not-exist.txt")); !os.IsNotExist(err) {
		t.Fatal("side effect occurred before whole-plan validation")
	}
}

func TestExecutePlanVerifiedWorkspaceMutation(t *testing.T) {
	c, s, tools := testCore(t)
	p := Plan{Summary:"write verified file", Steps:[]PlanStep{
		{Tool:"fs.write.workspace", Input:map[string]string{"path":"verified.txt","content":"hello"}},
		{Tool:"fs.read", Input:map[string]string{"path":"verified.txt"}},
	}}
...
func TestExecutePlanStopsAfterBlockedStep(t *testing.T) {
	c, _, tools := testCore(t)
	p := Plan{Summary:"stop on failure", Steps:[]PlanStep{
