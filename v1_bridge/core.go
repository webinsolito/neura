...
type ToolRegistry struct{workspace string;gate *CapabilityGate}
...
func NewToolRegistry(root string)(*ToolRegistry,error){abs,err:=filepath.Abs(root);if err!=nil{return nil,err};real,err:=filepath.EvalSymlinks(abs);if err==nil{abs=real};gate,err:=NewCapabilityGate();if err!=nil{return nil,err};return &ToolRegistry{workspace:abs,gate:gate},nil}
...
func (r *ToolRegistry)List()[]string{out:=[]string{"system.info","fs.list","fs.read"};if runtime.GOOS=="windows"{out=append(out,"windows.processes")};return out}
