...
func (r *ToolRegistry)List()[]string{out:=[]string{"system.info","fs.list","fs.read","fs.write.workspace","fs.mkdir.workspace","fs.move.workspace","fs.delete.workspace","fs.restore.workspace"};if runtime.GOOS=="windows"{out=append(out,"windows.processes","windows.app.launch")};return out}
func (r *ToolRegistry)Run(ctx context.Context,name string,input map[string]string,passport CapabilityPassport,purpose string)ToolResult{
	...
	case "windows.app.launch":
		rec,err:=r.launchAllowedWindowsApp(ctx,input["app"]);if err!=nil{return ToolResult{Tool:name,Error:err.Error()}};return ToolResult{Tool:name,Verified:rec.Verified,Data:rec}
	case "fs.read":
